...
void task_one::set_task_number(int i)
{
    ...
    {
    this->task_number = i;
    ui->label_5->setText("Equation: a*x*x+b*x+c");
    ui->label->setText("");
    ui->lineEdit_answer->setText("");
    ui->label_task->setText("Task "+ QString::number(task_number) + " bisection");
    ...
    ui->label_first->setText("First koeff: " + aString1);

    int value2= QRandomGenerator::global()->bounded(1, 20);
    QString aString2 = QString::number(value2);

    int value3= QRandomGenerator::global()->bounded(1, 20);
    QString aString3 = QString::number(value3);

    ui->label_second->setText("Second koeff: " + aString2);
    ui->label_c->setText("Third koeff: " + aString3);
...
void task_one::on_pushButton_send_clicked()  // sem sa treba pozriet a preverit ako to funguje
{
    ...
    QString third = ui->label_c->text();
    QStringList th = third.split(" ");
    QString third_guess = th[2];
    QString task_to_text = QString::number(task_number);
    SingletonClient::getInstance()->send_msg_to_server_check(task_to_text, first_guess, second_guess, third_guess);
}
...
void task_one::write_anser(QString answer) {
